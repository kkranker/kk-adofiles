...
{p 8 18 2}
{cmdab:memmax} [, options ]

{synoptset 20 tabbed}{...}
{synopthdr}
{synoptline}
{synopt:{opt min(integer)}}Specify minimum memory you require.{p_end}
{synopt:{opt max(integer)}}Specify maximum memory you require.{p_end}
{synopt:{opt units(b|k|m|g)}}Units of memory specification.{p_end}
{synopt:{opt buffer(integer)}}Leave small amount of space for operating system.{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
  Defaults: {opt max(2200)}, {opt units(m)}, {opt buffer(25)} (i.e. 2200m and 25m).  If {opt min} is not specified, memory allocation is increased from the current memory allotment.
  {p_end}
...
memory is not changed or (b) the program does not reach the {it:maximum} memory 
